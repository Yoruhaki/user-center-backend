...
def test_is_any_blank():
    ...
    with raises(TypeError, match=f"输入的参数需为 str 类型, {123} 为 {type(123)}"):
        StringUtils.is_any_blank("asd", "222", 123)
    with raises(TypeError, match=f"输入的参数需为 str 类型, {None} 为 {type(None)}"):
        StringUtils.is_any_blank("asd", "222", None)
    with raises(TypeError, match=f"输入的参数需为 str 类型, {12.5} 为 {type(12.5)}"):
...
def test_temp():
    print()
    print({"python"}.issubset({"1", "c++", "java"}))
